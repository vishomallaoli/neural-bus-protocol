...
class NeuralBUSPipeline:
    ...
    Flow: Image → Vision → Encoder → BUS → Decoder → LLM → Answer
    """
    ...
    def __init__(self, device: str = "cpu", use_mock: bool = False):
        ...
        # [2] Captioner
        print("\n[2/5] Loading Captioner...")
        self.captioner = MockCaptioner(device=self.device) if use_mock else Captioner(device=self.device)
        ...
        # Tokenizer (student) for text path
        if not use_mock:
    ...
    def __call__(
        ...
        Args:
            ...
            save_packet: optional path to save BUS packet
        ...
        # Training branch: keep grads through adapters; skip packet serialization
        ...
        # Mock training branch: fabricate logits w/ grad so KD works
